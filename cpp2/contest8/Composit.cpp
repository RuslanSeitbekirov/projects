...
//     std::string click(int x, int y) override {
//         // Для Label: если клик в пределах текста, возвращаем имя
//         Canvas c = render();
...
//     bool checked;
// public:
//     CheckBox(const std::string& text, bool checked): text(text), checked(checked) {
//     }
...
//     std::string click(int x, int y) override {
//         // Для CheckBox: если клик в пределах checkbox, возвращаем имя
//         Canvas c = render();
//         if (x >= 0 && x < c.width() && y >= 0 && y < c.height()) {
...
//             Canvas t = i->render();
//             if (curY > 0) {
//                 curY += 1; // отступ в одну строку между компонентами
//             }
//             c.draw(t, 0, curY);
//             curY += t.height();
//         }
...
//             // Проверяем попадание в вертикальные границы компонента
//             if (y >= curY && y < curY + t.height()) {
//                 // Передаём клик с относительными координатами
//                 return i->click(x, y - curY);
//             }
            
//             curY += t.height() + 1; // +1 для отступа между компонентами
            
//             // Если мы уже прошли точку клика по Y, дальше можно не искать
//             if (curY > y) {
//                 break;
//             }
//         }
